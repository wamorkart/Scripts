...
infile_data = inDir+"/data_2017.root"
tree_data = TChain()
...
infile_bkg = TFile(inDir+"data_mix_2017_even.root")

tree_bkg = infile_bkg.Get("Data_13TeV_H4GTag_0")
nentries_bkg = tree_bkg.GetEntries()

tree_bkg_out = TFile( inDir+"check.root","RECREATE")
...
for i in range(10000, nentries_bkg):
    if i%1000 == 0: print i
    tree_bkg.GetEntry(i)
